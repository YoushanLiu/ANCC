...
real(8), intent(in) :: delta, om0

real(8), intent(in) :: pred(npred,2)


real(8), intent(out) :: om1, Gt0
...
do i = 1, npred, 1
   x(i) = 2.d0*PI/pred(npred-i+1,1)
   y(i) = delta/pred(npred-i+1,2)
enddo

! get velocity for low integral boundary
call mspline(ip, npred, x, y, 0, 0.d0, 0, 0.d0)
...
call mspline(ip+1, npred, x, y, 0, 0.d0, 0, 0.d0)
