...
real(8), dimension(:), allocatable :: phgrc, trig1, omdom, ampdom
...
complex(8), dimension(:), allocatable :: pha_cor, env, spref
...
! spectra tapering
ome1 = min(ome, 2.d0*PI/minTpr)
omb1 = max(omb, 2.d0*PI/maxTpr)
...
do i = 1, ns, 1
   ...
   elseif (i > inde) then
      ...
      call msplint(ip+1, om0, omdom(i), pha_corr, ier)
      pha_cor(i) = cmplx(pha_corr, 0.d0)
   endif

   pha_cor(i) = exp(c1j*pha_cor(i))
...
if (0 /= ierr) then
   ...
   if (0 /= ier) then
      ...
      call trigger(grvel1(1:nfout2), om1(1:nfout2), nfout2, tresh, &
                             trig1(1:nfout2), ftrig1(1:nfout2), ier)
