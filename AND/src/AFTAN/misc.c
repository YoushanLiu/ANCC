               ...
               float *t0, float *seis, int *ierr)
{
    ...
    *p = ch;
    ...
    if (*sac)
    {
        ...
        *t0 = sach.b;
        /* The body */
        fread(seis, sizeof(float), *nsam, fd);
        if (iswap)
            swapn((unsigned char *)seis, (int)(sizeof(float)), *nsam);
        n = *nsam;
        /* make symmetric cross-correlaton */
        nn = (1.0 - *t0 / *dt) + 0.5;
        if ((2 * nn - 1) != n)
        ...
        for (i = 0; i < nn; i++)
            seis[n-i-1] = 0.50f*(seis[i] + seis[n-i-1]);
        for (i = 0; i < nn; i++)
            seis[i] = seis[nn-1+i];
        seis[0] *= 0.5f;
        //*n = nn;
 ...
 */
void printres_(float *xdt, float *delta, int *xnfout1, float **arr1,
               int *xnfout2, float **arr2, float *xtamp, int *xnrow,
               int *xncol, float **ampo, int *xierr, char *xname, char *xpref)
{
